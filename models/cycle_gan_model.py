...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            # self.model_names = ['G_A', 'G_B', 'G_AC', 'G_BC', 'D_AB', 'D_BC', 'D_AC']
        ...
        if not opt.no_dropout:
            print("________________________________")
            print("DROPOUT WAS ADDED!!")
            print("________________________________")
        ...
        if self.isTrain:  # define discriminators
            # self.netD_AB = networks.define_D(opt.output_nc*2, opt.ndf, opt.netD,
        ...
        if self.isTrain:
            ...
            #self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_B.parameters(),self.netD.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
            ...
            # self.optimizer_GA = torch.optim.Adam(itertools.chain(self.netG_A.parameters(), self.netG_AC.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
            # self.optimizer_GB = torch.optim.Adam(itertools.chain(self.netG_B.parameters(), self.netG_BC.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))

            # self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_AB.parameters(),self.netD_BC.parameters(),self.netD_AC.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    #     self.loss_D_AB = self.backward_D_basic(self.netD_AB, AB.detach(), BA.detach())
    ...
    def backward_D_BC(self):
        ...
        #conditional loss
        # BC = torch.cat((self.real_B,self.fake_BC),1)
        # realBC = torch.cat((self.real_B,self.real_C),1)
        # self.loss_D_BC = self.backward_D_basic(self.netD_BC, realBC.detach(), BC.detach())
    ...
    #             # GAN loss D_A(G_A(A))
    #     self.loss_G_A = self.criterionGAN(self.netD_AB(self.AtoB), False)
    #     # GAN loss D_B(G_B(B))
    #     self.loss_G_B = self.criterionGAN(self.netD_AB(self.BtoA), True)

    #     self.loss_G_AC = self.criterionGAN(self.netD_AC(self.fake_AC),True)

    #     #conditional loss
    #     # BC = torch.cat((self.real_B,self.fake_BC),1)
    #     # self.loss_G_BC = self.criterionGAN(self.netD_BC(BC),True)
    ...
    #     self.loss_pix = self.L1Loss(self.fake_BC, self.real_C) * lambda_A
    ...
    #     self.loss_G = 2*(self.loss_G_A + self.loss_G_B) + self.loss_G_AC + self.loss_G_BC + self.loss_cycle_AC + self.loss_cycle_BC + self.loss_pix
    ...
    #     self.loss_GA = self.loss_G_A + self.loss_G_AC  + self.loss_cycle_AC + self.loss_idt_AC
    #     # print(self.loss_G_A)
    ...
    #     self.loss_GB = self.loss_G_B + self.loss_G_BC + self.loss_cycle_BC + self.loss_pix + self.loss_idt_BC
    ...
    #     self.set_requires_grad([self.netG_A, self.netG_AC, self.netG_B, self.netG_BC], True)  
    ...
    #     # self.set_requires_grad([self.netD_AB,self.netD_BC, self.netD_AC], False)  # Ds require no gradients when optimizing Gs
    #     self.set_requires_grad([self.netG_B, self.netG_BC], False)  
    #     self.set_requires_grad([self.netG_A, self.netG_AC], True)  

    #     # print(self.fake_B.requires_grad)

    #     self.optimizer_GA.zero_grad()  # set G_A and G_B's gradients to zero
    #     self.backward_GA()             # calculate gradients for G_A and G_B
    #     self.optimizer_GA.step()       # update G_A and G_B's weights



    #     self.set_requires_grad([self.netG_B, self.netG_BC], True)  
    #     self.set_requires_grad([self.netG_A, self.netG_AC], False)  

    #     self.optimizer_GB.zero_grad()  # set G_A and G_B's gradients to zero
    #     self.backward_GB()             # calculate gradients for G_A and G_B
    #     self.optimizer_GB.step()       # update G_A and G_B's weights
    ...
    #     # self.set_requires_grad([self.netD_AB,self.netD_BC, self.netD_AC], True)
